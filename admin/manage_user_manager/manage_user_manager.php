...
<head>
    ...
    <title>Users_Orders</title>
    <link rel="stylesheet" href="../manage_user_manager/manage_user_manager.css">
...
<body class="bg-light" id="topregi">
    <div class="container bg-dark text-light p-3 rounded my-4">
        <div class="d-flex align-items-center justify-content-between">
            <h2><i class="bi bi-bag-check"></i>Users Order</h2>
        </div>
    </div>
    <?php
...
<div class="container mt-4 p-0">
		<table class="table table-hover text-center">
			<thead class="bg-dark text-light">
				<tr>
					...
					<th width="15%" scope="col">Order_id</th>
					<th width="10%" scope="col">Item_Name</th>
			...
			<tbody class="bg-white">
				...
				while ($fetch = mysqli_fetch_assoc($result)) {
					...
					echo <<<product
								...
								<button onclick="confirm_rem($fetch[Order_id])" class="btn btn-danger"><i class="bi bi-trash"></i></button>
								</td>
								</tr>
								product;
					$i++;
	...
	<script>
        function confirm_rem(Order_id) {
            if (confirm("Are you sure,you want to delete this item?")) {
                window.location.href = "crud.php?rem=" + Order_id;
            }
        }
    </script>
